...
class MidiBert(nn.Module):
    def __init__(self, bertConfig, e2w, w2e):
        ...
        self.mask_word_np = np.array([self.e2w[etype]['%s <MASK>' % etype] for etype in self.classes], dtype=np.int_)
        self.pad_word_np = np.array([self.e2w[etype]['%s <PAD>' % etype] for etype in self.classes], dtype=np.int_)
