...
def get_args():
    ...
    ### path setup ###
    parser.add_argument('--dict_file', type=str, default='music_project/bert/CP.pkl')
    parser.add_argument('--name', type=str, default='')
    parser.add_argument('--ckpt', default='bert/pretrain_model.ckpt')
...
def load_data(dataset, task):
    ...
    if dataset not in ['pop909', 'pianist8', 'emopia', 'composer']:
        print(f'Dataset {dataset} not supported')
        exit(1)

    X_train = np.load(os.path.join(data_root, f'{dataset}/{dataset}_cp_train.npy'), allow_pickle=True)
    X_val = np.load(os.path.join(data_root, f'{dataset}/{dataset}_cp_valid.npy'), allow_pickle=True)
    X_test = np.load(os.path.join(data_root, f'{dataset}/{dataset}_cp_test.npy'), allow_pickle=True)

    print('X_train: {}, X_valid: {}, X_test: {}'.format(X_train.shape, X_val.shape, X_test.shape))

    if dataset == 'pop909':
        y_train = np.load(os.path.join(data_root, f'{dataset}/{dataset}_train_{task[:3]}ans.npy'), allow_pickle=True)
        y_val = np.load(os.path.join(data_root, f'{dataset}/{dataset}_valid_{task[:3]}ans.npy'), allow_pickle=True)
        y_test = np.load(os.path.join(data_root, f'{dataset}/{dataset}_test_{task[:3]}ans.npy'), allow_pickle=True)
    else:
